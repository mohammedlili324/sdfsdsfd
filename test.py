...
def tryy():
    ...
    with use_scope(name='try', clear=True):
        def p(url):
            if url == int():
                return 'put the YouTube link, not numbers'
            if url in str(['hay','hi',"Hi",'Hallo','hallo','HI']):
                return "Hi ,moibot!"
            if url in str(['what is your name','what is your name?','what your name','What your name','WTYN','What is your name','Who are you ?','Who are you','who','Who','Who you']):
                return toast("My name is MoiBot Created by moieis",color='#4FB567')
            if url in str(['How are You','How Are You','WTS','how are you','how are y','h o r']):
                return "I'm fine, what about you !? ,moibot!"
            if url in str(['me','Me','ME','You mean me','mE']):
                return "Yes You !! Put a Link ,moibot!"
            if url in str(['good','Good','Notbad','notbad','NOTBAD']):
                return "Like moibot cool  ... "
            if url in str(['bad','badd','Bad','BAD','NotGood','Sad','saD','sAd','sAD']):
                return 'I hope you are better .. moibot!'
            if url in str(['f','fk','FK','Fuk']):
                return "Don't think about saying obscene words" ,toast('What you mean hmm ?',color="#473B71")
            if url in str(['fuck','Fuck','fUck','fuCk','fucK','MTF','mtf','Mtf','MTf','Wtf','WTF','WTff','wtff','wtf','fuck you','fuckk you','Fuck You']):
                return 'i missed you .....!' , time.sleep(0.3),toast('Fuck u From MoiBot Ok. !')
            if url in str(['help','Help','h','HELP','hlp','hp','HP','i need help','I need help']):
                return '.. Paste the video link to download ...️' ,toast('You can talk to the beta [MoiBot] version now !',color="#FF8B01")

            elif url == str():
                return "...I didn't understand I'm still learning from moieis...."
        try:
            url = input('', type=TEXT, required=True,validate=p,help_text='You can talk to MoiBot in case you need help ..!',datalist=['help','hi','and more .. '])
